...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "AtlNetwork") type = AtlNetwork;
    if(strType == "AtlNetworkVariable") type = AtlNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == AtlNetwork) s = "AtlNetwork";
    if(type == AtlNetworkVariable) s = "AtlNetworkVariable";
