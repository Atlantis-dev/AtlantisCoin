...
const char * const BITCOIN_CONF_FILENAME = "atl.conf";
const char * const BITCOIN_PID_FILENAME = "atld.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "atl" is a composite category enabling all Atl-related debug output
            if(ptrCategory->count(string("atl"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AtlCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AtlCore
    // Mac: ~/Library/Application Support/AtlCore
    // Unix: ~/.atlcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AtlCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AtlCore";
#else
    // Unix
    return pathRet / ".atlcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty atl.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override atl.conf
